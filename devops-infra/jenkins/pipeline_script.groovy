node {
    def APP_NAME_BACKEND = "gamification-backend"
    def APP_NAME_FRONTEND = "gamification-frontend"
    def IMAGE_TAG = "${env.BUILD_NUMBER}"
    def REPO_URL = "https://github.com/zalmanovmaxim-alt/devsecops-final-project.git"
    def DOCKER_REGISTRY = "your-ecr-registry-url"

    try {
        stage('1. Checkout Code') {
            echo "Manually cloning ${REPO_URL}..."
        ...
        stage('3. Test App') {
            ...
            // Assuming python3 is available. If using venv, might need activation.
            // Using 'pip3' might be safer if 'pip' is not mapped.
            sh "cd backend-api && pip3 install -r requirements.txt && pytest"
        }
